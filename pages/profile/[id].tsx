  ...
  return (
    <>
      ...
      <div className="mobile-container" style={{
        background: isExpert ? '#0e1e3c' : '#f5f7fb',
            ...
            <div style={{ padding: '0 16px' }}>
              ...
              {careers.length > 0 && (
                <DarkSection>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {careers.map((c, i) => (
                      <div key={i} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', padding: '12px 0', borderBottom: i < careers.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                        <span style={{ color: '#5a87be', fontSize: '16px', lineHeight: 1, flexShrink: 0, marginTop: '2px' }}>•</span>
                        <span style={{ color: 'rgba(255,255,255,0.82)', fontSize: '14px', lineHeight: 1.6, fontWeight: 500 }}>{c}</span>
                      </div>
                    ))}
                  </div>
                </DarkSection>
              ...
              <DarkSection>
                ...
                <div style={{ position: 'relative' }}>
                  <span style={{ position: 'absolute', top: '-8px', left: '-4px', color: 'rgba(90,135,190,0.5)', fontSize: '32px', fontFamily: 'Georgia, serif', lineHeight: 1 }}>"</span>
                  <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '14px', lineHeight: 1.9, paddingLeft: '18px', whiteSpace: 'pre-line' }}>
                    {displayIntro}
                  </p>
                </div>
              </DarkSection>
              ...
              {(agent?.email || agent?.kakao || agent?.blog || (agent as any)?.instagram || (agent as any)?.youtube) && (
                <DarkSection>
                  ...
                  <SnsInfoSection agent={agent} dark={true} />
                </DarkSection>
              )}

              <p style={{ textAlign: 'center', fontSize: '11px', color: 'rgba(255,255,255,0.18)', padding: '20px 0 8px', fontFamily: 'Cormorant Garamond, serif', letterSpacing: '2px' }}>
                INCAR PROFILE
              </p>
            </div>
          </div>
        ...
        {!isExpert && (
          <div>
            <div style={{ background: 'linear-gradient(180deg, #152a52 0%, #1e3a6e 100%)', padding: '48px 24px 72px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '14px' }}>
                <div style={{ width: '96px', height: '96px', borderRadius: '50%', overflow: 'hidden', background: 'rgba(255,255,255,0.08)', border: '2.5px solid rgba(255,255,255,0.28)', boxShadow: '0 8px 28px rgba(0,0,0,0.25)' }}>
                  {profile.photoUrl && <img src={profile.photoUrl} alt={agent?.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                </div>
              </div>
              <h1 style={{ textAlign: 'center', color: 'white', fontSize: '24px', fontWeight: 700, marginBottom: '4px' }}>{agent?.name}</h1>
              {[agent?.branch, agent?.position].filter(Boolean).length > 0 && (
                <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.55)', fontSize: '13px', marginBottom: '4px' }}>
            ...
            <div style={{ marginTop: '-32px', padding: '0 16px' }}>
              ...
              <p style={{ textAlign: 'center', fontSize: '11px', color: '#cbd5e0', padding: '16px 0 8px', fontFamily: 'Cormorant Garamond, serif', letterSpacing: '1px' }}>INCAR PROFILE</p>
            </div>
          </div>
        )}
...
function DarkSection({ children }: { children: React.ReactNode }) {
  return <div style={{ marginBottom: '12px', padding: '20px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px' }}>{children}</div>;
}
function DarkLabel({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: '12px' }}>{children}</p>;
}
const darkRow: React.CSSProperties = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 0', textDecoration: 'none' };
